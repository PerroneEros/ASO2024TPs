...
def ver_ordenes():
    ...
    # Consulta SQL para filtrar órdenes por ID del cliente
    consulta = '''
    SELECT d.ID_Detalle, d.ID_Orden, o.Fecha, c.Nombre AS Nombre_Cliente, 
           d.ID_Producto, p.Nombre AS Nombre_Producto, d.Cantidad
    ...
    try:
        ...
        if detalles:
            # Mostrar los detalles de las órdenes filtradas
            for detalle in detalles:
                print(f"ID_Detalle: {detalle[0]}, ID_Orden: {detalle[1]}, Fecha: {detalle[2]}, "
                      f"Cliente: {detalle[3]}, ID_Producto: {detalle[4]}, Producto: {detalle[5]}, "
                      f"Cantidad: {detalle[6]}")
        else:
